...
$.mix({
   ...
   * 遍历文件树,收集目录与文件,并包含自身
   * p 为路径
   * cb 为最终回调，它将接受两个参数files, dirs，所有文件列表与所有目录列表
   * opts 为可选的配置对象，里面参数：
   *    sync  表示是否同步
   *    one   表示是否找到一个就终于遍历
   * filter 表示过滤函数，如果函数返回true则收录
  ...
  /**
   * 目录对拷,可以跨分区拷贝
   * @param {*} old 一个目录的路径，String,以“/”隔开
   * @param {*} neo 一个目录的路径，String,以“/”隔开
   * @param {*} cb 可选，回调
   */
  cpdirSync: new (function (old, neo, cb) {
    ...
    return function (old, neo, cb) {
      //把当前目录里面的东西拷贝到新目录下（不存在就创建）
